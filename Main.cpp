                     ...
                     int       nCmdShow)
{
	...
	while (parsed != NULL)
	{
		cout << "ARGV[" << argv.size() << "]: " << parsed << endl;
		argv.push_back(parsed);
		parsed = strtok(NULL, tok);
	}
	switch (argv.size())
	{
	case 0:
		cout << "Image name not set" << endl;
	case 1:
		cout << "Program mode not set" << endl;
	case 2:
		cout << "Scale ratio not set" << endl;
	case 3:
		cout << "Alias indicator not set" << endl;
		break;
	default:
		mode = stoi(argv[1].c_str());
		scale = stold(argv[2].c_str());
	...
	inImage.setHeight(h);
	/********************/
	inImage.setImagePath("miamibeach.rgb");
